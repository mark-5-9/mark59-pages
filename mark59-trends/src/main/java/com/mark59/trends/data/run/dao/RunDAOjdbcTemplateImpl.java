...
//						System.out.println("populating run_times for trending page (a std run found): " + row.get("RUN_TIME")  ) ;
	...
	public String runsSQL(String application, String sqlSelectRunLike, String reqSqlSelectRunNotLike,
		...
		// Escape all user inputs to prevent SQL injection before string replacement
		// Note: This method returns a SQL string (not a PreparedStatement) so manual escaping is critical
		String escapedApplication = UtilsTrends.escapeSqlString(application);
		String escapedSqlSelectRunLike = UtilsTrends.escapeSqlString(sqlSelectRunLike.replace(".","").replace("T","").replace(":","").trim());
		String escapedReqSqlSelectRunNotLike = UtilsTrends.escapeSqlString(reqSqlSelectRunNotLike.replace(".","").replace("T","").replace(":","").trim());

		sql = sql.replace(":application", "'" + escapedApplication + "' ")
				 .replace(":sqlSelectRunLike", "'%" + escapedSqlSelectRunLike + "%'" )
				 .replace(":reqSqlSelectRunNotLike", "'%" + escapedReqSqlSelectRunNotLike + "%'" );
